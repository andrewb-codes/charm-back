package ru.andrewb.charm.pool.model;
...
public class ProxyConnection implements Connection {
    ...
    private final AtomicBoolean inPool = new AtomicBoolean(true);
    ...
    public void close() throws SQLException {
        if (!inPool.compareAndSet(false, true)) {
            return;
        }

        resetState();
        pool.offer(this);
        log.debug("ProxyConnection returned to pool, size: {}", pool.size());
    }
    ...
    public boolean isClosed() throws SQLException {
        return inPool.get() || delegate.isClosed();
    }
